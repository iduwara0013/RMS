...
export default function ApplyPage() {
  ...
  return <main className="public-vacancies">
    ...
    <section className="application-content"><div className="application-description"><p className="eyebrow eyebrow--red">Role overview</p><h2>{vacancy.title}</h2><p>{vacancy.description}</p></div>{submitted ? <div className="application-success"><div className="dashboard-card__icon">✓</div><h2>Application received</h2><p>{message}</p><Link className="vacancy-apply" href="/vacancies">Return to vacancies <span>→</span></Link></div> : <form className="application-form" onSubmit={submit}><h2>Submit your application</h2><p>Complete your details and attach your CV to apply.</p><input required placeholder="NIC number" value={form.nic} onChange={(event) => setForm({ ...form, nic: event.target.value })} /><input required placeholder="Full name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><input required type="email" placeholder="Email address" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /><input required type="tel" placeholder="Phone number" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /><textarea required placeholder="Address" value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} /><label className="cv-upload">CV (PDF, DOC or DOCX)<input required type="file" accept=".pdf,.doc,.docx" onChange={(event) => setCv(event.target.files?.[0] ?? null)} /></label><button className="vacancy-apply" type="submit">Submit application <span>→</span></button>{message && <p className="form-message form-message--error">{message}</p>}</form>}</section>
